...
This is a guide to setting up PredictionIO and the Universal Recommender in a 3 node cluster. This is consistent with simpler single machine setups for dev environments or for non-clustered setups. In this guide all services are setup with multiple or standby masters in true clustered mode. To make  High Availability complete, a secondary master would need to be setup for HDFS (not described here). Elasticsearch and HBase are setup in High Availabiltiy mode (HA) using this guide.
